...
class UserMembershipController extends Controller
{
        ...
        if($user->save()){
            ...
            if($membership_card->save()){
                if($membership_request->delete()){
                    ...
                    return response()->json(['message' => 'Uspješno kreirani član biblioteke i članska iskaznica.', 'otp' => $generated_password], 200);
                }else{
    ...
    public function destroy(Request $request){
        ...
        if(ModelsRequest::destroy($request->membership_request_id)){
            Mail::to($membership_request->email)->send(new MembershipRequestInfo(['name' => $membership_request->name . " " . $membership_request->surname, 'message' => $request->message, 'type' => 'failure'], 'failure'));
